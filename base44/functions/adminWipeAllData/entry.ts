...
Deno.serve(async (req) => {
  try {
    ...
    // SDK >= 0.8.x: auth.me() devolve o objecto directamente (ou null),
    // não um {data, error} wrapper.
    const user = await base44.auth.me();
    if (!user?.id) {
      return Response.json({ error: 'Acesso não autorizado.' }, { status: 401 });
    }
    // Verificar `role` (gerido pela plataforma), não `user_type`
    ...
    return Response.json({ success: true, logs: deletionLogs });

  } catch (error) {
    console.error('❌ Erro crítico durante a limpeza de dados:', error);
    return Response.json({ error: 'Erro no servidor durante a limpeza.', details: (error as Error).message }, { status: 500 });
  }
});
