    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 w-full">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">{t(lang,"level")}</p>
            <p className="text-sm font-bold text-gray-900">{current.name}</p>
          </div>
        </div>
        <div className="text-right">
          <p className="text-2xl font-black text-gray-900">{xp.toLocaleString()}</p>
          <p className="text-xs text-gray-400">{t(lang,"xpPoints")}</p>
        </div>
      </div>
      <div className="w-full bg-gray-100 rounded-full h-2 mb-1">
        <div className={"h-2 rounded-full bg-gradient-to-r " + current.color + " transition-all duration-500"} style={{ width: progress + "%" }} />
      </div>
      <div className="flex justify-between text-xs text-gray-400">
        <span>{current.name}</span>
        {nextLevel ? (
          <span>{nextLevel.min.toLocaleString()} XP → {nextLevel.name}</span>
        ) : (
          <span>🏆 {t(lang,"maxLevel")}</span>
        )}
      </div>
    </div>
  );
